...
const DefaultSensor: React.FC<Sensor> = ({ name, value, max, unity }) => {
  ...
  return (
    <div className="relative flex flex-col justify-between bg-white shadow rounded-lg w-full h-full p-4" >
      {/* Nombre en la parte superior */}
      <h3 className="text-left text-lg font-semibold text-gray-700">{name}</h3>

      {/* Contenedor del "líquido" */}
      <div className="relative flex justify-center items-end h-full text-black w-full bg-gray-300 rounded-lg overflow-hidden mt-4 shadow-inner">
...
const prueba:any = ({ sensor, selectedSensor }) => (
    <>
    casa
    </>
)
...
const CarouselItem: /*  React.FC<{ sensor: Sensor, selectedSensor: boolean }> */ any =
    ...
    return (
      <div
        className={`flex-shrink-0 w-full h-full rounded-md ${
          selectedSensor ? "border-4 border-blue-500" : ""}
        ${ sensor.sensor_code === 'PH' ? "relative flex flex-col justify-between bg-white shadow rounded-lg w-full h-40 p-4" : null} 
        `
